...
const char* WIFI_PASSWORD = "riham035";
const char* MQTT_BROKER = "51.83.98.100"; 
const int MQTT_PORT = 1883;
const char* MQTT_CLIENT_ID = "esp32cam_"; 
const char* MQTT_TOPIC = "breeze/cameras/"; 
...
int unsigned long FRAME_INTERVAL = 200; 
int frameCount = 0;
int videoQuality = 12; 
...
void handleStream() {
  ...
  while (client.connected()) {
    ...
    if (!fb) {
      Serial.println(" Camera capture failed (HTTP stream)");
      break;
    }
...
void setup() {
  ...
  server.begin();
  Serial.println(" HTTP server started on /stream");
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if (doc.containsKey("command")) {
    ...
    if (command == "restart") {
      ...
      FRAME_INTERVAL = max(100, interval); 
...
void captureAndSendFrame() {
  ...
  if (!isKeyFrame && fb->len > 5000) {
    ...
    if (s) {
      ...
      s->set_quality(s, min(25, originalQuality + 10));
